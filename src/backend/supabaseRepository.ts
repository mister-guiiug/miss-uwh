 ...
 * commence donc par `await getSupabase()`.
 *
...
export async function fetchClub(): Promise<(Club & { id: string }) | null> {
  ...
  const rows = unwrap(await sb.from('clubs').select('*').limit(1)) as ClubRow[];
...
export async function fetchAiConfig(): Promise<AiClubConfig | null> {
  ...
  const sb = await getSupabase();
  const { data, error } = await sb.from('ai_config').select('*').limit(1);
  if (error) {
...
export async function fetchSeasons(): Promise<Season[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('seasons').select('*').order('label')
  ) as SeasonRow[];
...
export async function fetchEvents(): Promise<EventLedger[]> {
  const sb = await getSupabase();
  const rows = unwrap(await sb.from('events').select('*')) as EventRow[];
...
export async function fetchRecurrings(): Promise<RecurringTemplate[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('recurrings').select('*').order('label')
  ) as RecurringRow[];
...
export async function fetchAdherents(): Promise<Adherent[]> {
  const sb = await getSupabase();
  const rows = unwrap(await sb.from('adherents').select('*')) as AdherentRow[];
...
export async function fetchCustomCategories(): Promise<Category[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('categories').select('*').eq('custom', true)
  ) as CategoryRow[];
...
export async function fetchGuardians(): Promise<Guardian[]> {
  const sb = await getSupabase();
  const rows = unwrap(await sb.from('guardians').select('*')) as GuardianRow[];
...
export async function fetchClubEvents(): Promise<ClubEvent[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('club_events').select('*').order('date')
  ) as ClubEventRow[];
...
export async function fetchAnnouncements(): Promise<Announcement[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('announcements').select('*').order('date')
  ) as AnnouncementRow[];
...
export async function fetchTournaments(): Promise<Tournament[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('tournaments').select('*').order('date')
  ) as TournamentRow[];
...
export async function fetchTrainingSessions(): Promise<TrainingSession[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('training_sessions').select('*').order('date')
...
export async function fetchExercises(): Promise<Exercise[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('exercises').select('*').order('name')
  ) as ExerciseRow[];
...
export async function fetchStrategies(): Promise<Strategy[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('strategies').select('*').order('name')
  ) as StrategyRow[];
...
export async function fetchReferees(): Promise<Referee[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('referees').select('*').order('name')
  ) as RefereeRow[];
...
export async function fetchPhotoAlbums(): Promise<PhotoAlbum[]> {
  const sb = await getSupabase();
  const rows = unwrap(
    await sb.from('photo_albums').select('*').order('date')
  ) as PhotoAlbumRow[];
...
export async function upsertEntry(entry: JournalEntry): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('entries').upsert(entryToUpsertRow(entry), {
      onConflict: 'id',
    })
  );
}
...
export async function upsertEntries(entries: JournalEntry[]): Promise<void> {
  ...
  const sb = await getSupabase();
  unwrap(
    await sb.from('entries').upsert(entries.map(entryToUpsertRow), {
      onConflict: 'id',
    })
  );
}
...
export async function upsertSeason(season: Season): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('seasons').upsert(seasonToUpsertRow(season), {
      onConflict: 'id',
    })
  );
}
...
export async function upsertEvent(event: EventLedger): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('events').upsert(eventToRow(event), { onConflict: 'id' })
  );
}
...
export async function upsertRecurring(t: RecurringTemplate): Promise<void> {
  ...
  const sb = await getSupabase();
  unwrap(
    await sb.from('recurrings').upsert(recurringToUpsertRow(t, clubId), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteRecurring(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('recurrings').delete().eq('id', id));
}

export async function upsertAdherent(a: Adherent): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('adherents').upsert(adherentToUpsertRow(a), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteAdherent(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('adherents').delete().eq('id', id));
}

export async function upsertGuardian(g: Guardian): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('guardians').upsert(guardianToUpsertRow(g), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteGuardian(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('guardians').delete().eq('id', id));
}

export async function upsertClubEvent(e: ClubEvent): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('club_events').upsert(clubEventToUpsertRow(e), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteClubEvent(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('club_events').delete().eq('id', id));
}

export async function upsertAnnouncement(a: Announcement): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('announcements').upsert(announcementToUpsertRow(a), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteAnnouncement(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('announcements').delete().eq('id', id));
}

export async function upsertTournament(t: Tournament): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('tournaments').upsert(tournamentToUpsertRow(t), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteTournament(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('tournaments').delete().eq('id', id));
}

export async function upsertTrainingSession(s: TrainingSession): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('training_sessions').upsert(trainingSessionToUpsertRow(s), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteTrainingSession(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('training_sessions').delete().eq('id', id));
}

export async function upsertExercise(e: Exercise): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('exercises').upsert(exerciseToUpsertRow(e), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteExercise(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('exercises').delete().eq('id', id));
}

export async function upsertStrategy(s: Strategy): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('strategies').upsert(strategyToUpsertRow(s), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteStrategy(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('strategies').delete().eq('id', id));
}

export async function upsertReferee(r: Referee): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('referees').upsert(refereeToUpsertRow(r), {
      onConflict: 'id',
    })
  );
}
...
export async function deleteReferee(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('referees').delete().eq('id', id));
}

export async function upsertPhotoAlbum(a: PhotoAlbum): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('photo_albums').upsert(photoAlbumToUpsertRow(a), {
      onConflict: 'id',
    })
  );
}
...
export async function deletePhotoAlbum(id: string): Promise<void> {
  const sb = await getSupabase();
  unwrap(await sb.from('photo_albums').delete().eq('id', id));
}

export async function upsertCustomCategory(c: Category): Promise<void> {
  const sb = await getSupabase();
  unwrap(
    await sb.from('categories').upsert(customCategoryToUpsertRow(c), {
      onConflict: 'code',
    })
  );
}
...
export async function closeSeasonRpc(id: string): Promise<void> {
  const sb = await getSupabase();
  const { error } = await sb.rpc('close_season', { p_season: id });
...
export async function reopenSeasonRpc(
  ...
  const sb = await getSupabase();
  const { error } = await sb.rpc('reopen_season', {
    p_season: id,
    p_reason: reason,
  });
  if (error) throw new Error(error.message);
